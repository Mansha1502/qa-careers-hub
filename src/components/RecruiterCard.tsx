    ...
    <div className="card-shadow flex flex-col gap-3.5 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 transition-all duration-200 hover:card-shadow-hover hover:-translate-y-[3px] hover:border-[var(--border-strong)]">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <h3 className="font-display truncate text-[16px] font-medium text-[var(--foreground)]">
            {recruiter.company}
          </h3>
          <p className="truncate text-[13.5px] text-[var(--muted)]">{recruiter.location}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {recruiter.hiresFor.map((region) => (
          <span
            key={region}
            className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium ${regionStyles[region]}`}
          >
            {region}
          </span>
        ))}
      ...
      {recruiter.note && (
        <p className="text-[13.5px] leading-relaxed text-[var(--muted)]">{recruiter.note}</p>
      )}
      ...
      <div className="mt-auto pt-1">
        ...
        {recruiter.contactLabel && (
          <p className="mt-1.5 text-xs text-[var(--muted-soft)]">{recruiter.contactLabel}</p>
        )}
      </div>
    </div>
  );
